    ...
    <div className="p-4 space-y-4">
        ...
        {metrics.map((metric, i) => (
          <CommandCard key={i} accentColor={metric.color}>
            <div className="space-y-2">
              <div className="text-xs uppercase tracking-[0.08em] text-s3m-text-tertiary">
                {metric.label}
              </div>
              <div className="font-mono text-xl font-semibold" style={{ color: metric.color }}>
                {metric.value}
              </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="relative bg-s3m-card border border-s3m-border-default rounded p-4">
          ...
          <div className="mb-4 flex items-center gap-2">
            ...
            <span className="text-xs uppercase tracking-[0.08em] text-s3m-text-secondary font-semibold">
              ATTENTION REQUIRED
            </span>
          </div>
                ...
                >
                  <div className="flex items-start justify-between mb-1">
                    <div className="flex items-center gap-2">
                      <span className="font-mono text-base text-s3m-text-tertiary">
                        {priority.id}
                      </span>
                      <span
                        className="text-base uppercase tracking-wider font-semibold px-1.5 py-0.5 rounded"
                        style={{
                    ...
                    {priority.risk && (
                      <span className="font-mono text-xs text-s3m-critical">
                        Risk {priority.risk}
                      </span>
                    )}
                  </div>
                  <div className="text-sm text-s3m-text-primary mb-1">
                    {priority.title}
                  </div>
                  <div className="text-xs text-s3m-text-tertiary">
                    {priority.source || ''}
        ...
        <div className="relative bg-s3m-card border border-s3m-border-default rounded p-4">
          ...
          {expandedInbox && (
            <div className="space-y-4">
              {/* Pending Approvals */}
              <div>
                <div className="text-base uppercase tracking-wider text-s3m-text-tertiary mb-2 flex items-center gap-2">
                  ...
                  {inboxItems.pendingApprovals.map((item) => (
                    <div key={item.id} className="bg-s3m-elevated rounded p-2 hover:border-cyber-cyan border border-transparent transition-all cursor-pointer">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="font-mono text-base text-s3m-text-tertiary">{item.id}</span>
                        <span
                        ...
                        <span className="text-xs text-s3m-text-tertiary">{item.authority}</span>
                      </div>
                      <div className="text-xs text-s3m-text-primary mb-1">{item.title}</div>
                      <div className="text-base text-s3m-text-tertiary">{item.time}</div>
                    </div>
                  ))}
                </div>
              </div>
                ...
                <div className="space-y-2">
                  {inboxItems.crossDomainAlerts.map((item) => (
                    <div key={item.id} className="bg-s3m-elevated rounded p-2 hover:border-cyber-cyan border border-transparent transition-all cursor-pointer">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="font-mono text-base text-s3m-text-tertiary">{item.id}</span>
                        <span className="text-xs text-cyber-cyan font-mono">{item.confidence}%</span>
                      </div>
                      <div className="text-xs text-s3m-text-primary mb-1">{item.title}</div>
                      <div className="flex gap-1 mb-1">
                        {item.domains.map((domain, i) => (
                          <span key={i} className="text-xs uppercase px-1.5 py-0.5 rounded bg-cyber-cyan/20 text-cyber-cyan">
                            {domain}
                          </span>
                        ))}
                      </div>
                ...
                <div className="space-y-2">
                  {inboxItems.escalations.map((item) => (
                    <div key={item.id} className="bg-s3m-elevated rounded p-2 hover:border-cyber-cyan border border-transparent transition-all cursor-pointer">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="font-mono text-base text-s3m-text-tertiary">{item.id}</span>
                        <span className="text-xs uppercase tracking-wider font-semibold px-1.5 py-0.5 rounded bg-s3m-critical/20 text-s3m-critical">
                          {item.level}
                        </span>
                      </div>
                      <div className="text-xs text-s3m-text-primary mb-1">{item.title}</div>
                      <div className="text-base text-s3m-text-tertiary mb-1">{item.reason}</div>
                      <div className="text-base text-s3m-text-tertiary">{item.time}</div>
                    </div>
                  ))}
                </div>
                ...
                <div className="space-y-2">
                  {inboxItems.shiftChanges.map((item, i) => (
                    <div key={i} className="bg-s3m-elevated rounded p-2">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="text-base uppercase text-cyber-cyan font-semibold">{item.category}</span>
                        <span
                          className="text-xs uppercase tracking-wider font-semibold px-1.5 py-0.5 rounded"
                          style={{
                      ...
                      <div className="text-xs text-s3m-text-primary">{item.change}</div>
                    </div>
                  ))}
                </div>
              </div>
      ...
      <div className="relative bg-s3m-card border border-s3m-border-default rounded p-4">
        ...
        >
          ...
          <div className="flex-1" />
          <span className="font-mono text-xs" style={{ color: '#8A5CFF' }}>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-3">
              <div>
                ...
                <div className="bg-s3m-elevated rounded p-3 text-base text-s3m-text-primary leading-relaxed">
              ...
              {/* ROE Profile */}
              <div>
                <div className="text-base uppercase tracking-wider text-s3m-text-tertiary mb-2">
                  ACTIVE ROE PROFILE
                </div>
                <div className="bg-s3m-elevated rounded p-3">
                  <div className="flex items-center gap-2 mb-2">
                    <span className="text-sm font-semibold text-cyber-cyan">{operationalDirectives.roeProfile.name}</span>
                    <span className="text-xs uppercase tracking-wider px-1.5 py-0.5 rounded bg-cyber-green/20 text-cyber-green">
                      {operationalDirectives.roeProfile.status}
                    </span>
                  </div>
                  <div className="text-base text-s3m-text-tertiary mb-2">Updated: {operationalDirectives.roeProfile.lastUpdate}</div>
                  <div className="space-y-1">
                    {operationalDirectives.roeProfile.keyProvisions.map((provision, i) => (
                      <div key={i} className="text-xs text-s3m-text-primary flex items-start gap-2">
              ...
              <div>
                ...
                <div className="space-y-2">
                  {operationalDirectives.theaterConstraints.map((constraint, i) => (
                    <div key={i} className="bg-s3m-elevated rounded p-2">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="text-xs uppercase tracking-wider px-1.5 py-0.5 rounded bg-s3m-warning/20 text-s3m-warning">
                          {constraint.type}
                        </span>
                        <span className="text-xs uppercase tracking-wider px-1.5 py-0.5 rounded bg-cyber-green/20 text-cyber-green">
                          {constraint.status}
                      ...
                      <div className="text-xs text-s3m-text-primary">{constraint.constraint}</div>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="space-y-2">
                  {operationalDirectives.delegatedAuthorities.map((auth, i) => (
                    <div key={i} className="bg-s3m-elevated rounded p-2">
                      <div className="text-xs font-semibold text-s3m-text-primary mb-1">{auth.authority}</div>
                      <div className="text-base text-cyber-cyan mb-1">→ {auth.delegatedTo}</div>
                      <div className="text-base text-s3m-text-tertiary">{auth.conditions}</div>
                    </div>
                  ))}
                </div>
              </div>
                ...
                <div className="space-y-2">
                  {operationalDirectives.protectedAssets.map((asset) => (
                    <div key={asset.id} className="bg-s3m-elevated rounded p-2 border-l-2 border-s3m-critical">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="font-mono text-base text-s3m-text-tertiary">{asset.id}</span>
                        <span
                          className="text-xs uppercase tracking-wider font-semibold px-1.5 py-0.5 rounded"
                          style={{
                            color: '#FF3366',
                      ...
                      <div className="text-xs font-semibold text-s3m-text-primary mb-1">{asset.name}</div>
                      <div className="flex items-center gap-2 text-base text-s3m-text-tertiary">
            ...
            {actionBoardItems.map((item) => (
              <div key={item.id} className="bg-s3m-elevated border border-s3m-border-default rounded p-4">
                <div className="flex items-start justify-between mb-3">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="font-mono text-xs text-s3m-text-tertiary">{item.id}</span>
                      <span className="text-xs uppercase tracking-wider px-1.5 py-0.5 rounded bg-cyber-cyan/20 text-cyber-cyan">
                        {item.type}
                      </span>
                      <span
                        className="text-xs uppercase tracking-wider font-semibold px-1.5 py-0.5 rounded"
                    ...
                    <div className="text-base font-semibold text-s3m-text-primary mb-2">{item.title}</div>
                    <div className="text-xs text-s3m-text-tertiary mb-1">Requestor: {item.requestor}</div>
                    <div className="text-base text-s3m-text-primary leading-relaxed mb-3">{item.description}</div>

                    <div className="grid grid-cols-2 gap-3 mb-3">
                      <div className="bg-s3m-card rounded p-2">
                        <div className="text-xs uppercase tracking-wider text-s3m-text-tertiary mb-1">LEGAL NOTE</div>
                        <div className="text-xs text-s3m-text-primary">{item.legalNote}</div>
                      </div>
                      <div className="bg-s3m-card rounded p-2">
                        <div className="text-xs uppercase tracking-wider text-s3m-text-tertiary mb-1">POLICY NOTE</div>
                        <div className="text-xs text-s3m-text-primary">{item.policyNote}</div>
                      </div>
                    </div>

                    <div className="flex gap-2">
                      <button className="px-3 py-1.5 rounded bg-cyber-green/20 text-cyber-green hover:bg-cyber-green/30 text-xs uppercase tracking-wider font-semibold transition-colors">
                        APPROVE
                      </button>
                      <button className="px-3 py-1.5 rounded bg-s3m-critical/20 text-s3m-critical hover:bg-s3m-critical/30 text-xs uppercase tracking-wider font-semibold transition-colors">
                        REJECT
                      </button>
                      <button className="px-3 py-1.5 rounded bg-s3m-warning/20 text-s3m-warning hover:bg-s3m-warning/30 text-xs uppercase tracking-wider font-semibold transition-colors">
                        DEFER
                      </button>
                      <button className="px-3 py-1.5 rounded bg-cyber-cyan/20 text-cyber-cyan hover:bg-cyber-cyan/30 text-xs uppercase tracking-wider font-semibold transition-colors">
                        ASSIGN TO STAFF
                      </button>
                      <button className="px-3 py-1.5 rounded bg-cyber-purple/20 hover:bg-cyber-purple/30 text-xs uppercase tracking-wider font-semibold transition-colors" style={{ color: '#8A5CFF' }}>
                        ISSUE FRAGO
                      </button>
                    </div>
                  </div>
      ...
      <div className="relative bg-s3m-card border border-s3m-border-default rounded p-4">
        ...
        <div className="mb-4 flex items-center gap-2">
          ...
          <div className="flex-1" />
          <span className="font-mono text-xs text-s3m-success">
              ...
              >
                <div className="flex items-start gap-2 mb-2">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="font-mono text-xs text-s3m-text-tertiary mb-1">
                      {agent.id}
                    </div>
                    <div className="text-base font-semibold text-s3m-text-primary truncate">
                      {agent.name}
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-1">
                  <div
                    className="text-base uppercase tracking-wider font-semibold px-1.5 py-0.5 rounded inline-block"
                    style={{
                      color: statusColor,
                  ...
                  <div className="text-xs text-s3m-text-tertiary font-mono">
                    ↑ {agent.uptime}
                  </div>
                </div>
              </div>
                  ...
                  <div className="p-6 border-b border-s3m-border-default flex items-center gap-4">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-1">
                        <span className="font-mono text-sm text-s3m-text-tertiary">
                          {agent.id}
                        </span>
                        <span
                          className="text-xs uppercase tracking-wider font-semibold px-2 py-1 rounded"
                          style={{
                            color: statusColor,
                            backgroundColor: `${statusColor}20`
                          }}
                      ...
                      <div className="text-xl font-bold text-cyber-cyan font-display tracking-wider">
                        {agent.name}
                      </div>
                      <div className="text-sm text-s3m-text-secondary">
                        {agent.type}
                      </div>
                    </div>
                    <button
                    ...
                    <div className="grid grid-cols-2 gap-6">
                      ...
                      <div className="space-y-4">
                        <div className="relative bg-s3m-card border border-s3m-border-default rounded p-4">
                          ...
                          <div className="mb-3">
                            <span className="text-xs uppercase tracking-[0.08em] text-s3m-text-tertiary font-semibold">
                              CURRENT FUNCTION
                            </span>
                          </div>
                          <div className="text-base text-s3m-text-primary leading-relaxed">
                            {agent.function}
                          </div>
                        </div>
                        ...
                        <div className="relative bg-s3m-card border border-s3m-border-default rounded p-4">
                          <div className="space-y-3">
                            <div>
                              <div className="text-xs uppercase tracking-[0.08em] text-s3m-text-tertiary mb-1">
                                LOCATION
                              </div>
                              <div className="text-sm text-s3m-text-primary font-mono">
                                {agent.location}
                              </div>
                            </div>
                            <div>
                            ...
                            <div>
                              ...
                              <div className="text-sm text-s3m-text-primary">
                                {agent.tasking}
                              </div>
                            </div>
                            <div>
                      ...
                      <div className="relative bg-s3m-card border border-cyber-cyan rounded p-4">
                        <CornerBrackets color="#00F0FF" />
                        <div className="mb-4">
                          <span className="text-base uppercase tracking-[0.08em] text-cyber-cyan font-semibold">
                        ...
                        <div className="space-y-4">
                          <div>
                            <label className="text-xs uppercase tracking-[0.08em] text-s3m-text-tertiary mb-2 block">
                              MISSION TYPE
                            </label>
                            <select className="w-full bg-s3m-elevated border border-s3m-border-default rounded px-3 py-2 text-sm text-s3m-text-primary focus:border-cyber-cyan focus:outline-none">
                          ...
                          <div>
                            ...
                            <input
                              ...
                              className="w-full bg-s3m-elevated border border-s3m-border-default rounded px-3 py-2 text-sm text-s3m-text-primary placeholder:text-s3m-text-tertiary focus:border-cyber-cyan focus:outline-none"
                            />
                          </div>
                          ...
                          <div>
                            ...
                            <textarea
                              ...
                              className="w-full bg-s3m-elevated border border-s3m-border-default rounded px-3 py-2 text-sm text-s3m-text-primary placeholder:text-s3m-text-tertiary focus:border-cyber-cyan focus:outline-none resize-none"
                            />
                          </div>
                          ...
                          <div>
                            ...
                            <div className="grid grid-cols-3 gap-2">
                              <input
                                type="number"
                                placeholder="HH"
                                className="bg-s3m-elevated border border-s3m-border-default rounded px-3 py-2 text-sm text-s3m-text-primary placeholder:text-s3m-text-tertiary focus:border-cyber-cyan focus:outline-none text-center"
                              />
                              <input
                                type="number"
                                placeholder="MM"
                          ...
                          <div className="flex gap-2 pt-2">
                            <button className="flex-1 bg-cyber-cyan hover:bg-cyber-cyan/80 text-cyber-deep font-semibold py-2.5 px-4 rounded text-base uppercase tracking-wider transition-colors">
                              DEPLOY MISSION
                            </button>
                            <button className="flex-1 bg-s3m-elevated hover:bg-s3m-card text-s3m-text-secondary font-semibold py-2.5 px-4 rounded text-base uppercase tracking-wider transition-colors border border-s3m-border-default">
                              SAVE DRAFT
                            </button>
                          </div>
                        </div>
