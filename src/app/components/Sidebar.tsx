...
import { Home, Target, CheckSquare, AlertTriangle, Map, Package, Activity, Shield, Play, Radio, Eye, Monitor } from 'lucide-react';
...
export function Sidebar() {
  ...
  const isDemoRoomRoute = location.pathname === '/demo-room';
  ...
  return (
    <div className="w-48 bg-cyber-deep/50 border-r border-cyber-glass-border flex flex-col py-4 gap-1" style={{ backdropFilter: 'blur(10px)' }}>
      ...
      >
        ...
        <Monitor className="w-5 h-5 flex-shrink-0" style={isDemoRoomRoute ? { filter: 'drop-shadow(0 0 4px rgba(0, 240, 255, 0.8))' } : {}} />
        <span className={`text-[11px] font-semibold uppercase tracking-wider ${
          isDemoRoomRoute ? 'text-cyber-cyan' : 'text-cyber-text-tertiary'
        }`} style={isDemoRoomRoute ? { textShadow: '0 0 8px rgba(0, 240, 255, 0.6)' } : {}}>
          DEMO ROOM
        </span>
      </button>
      ...
      {workspaces.map((workspace) => {
        ...
        const isActive = !isDemoRoomRoute && activeWorkspace === workspace;
